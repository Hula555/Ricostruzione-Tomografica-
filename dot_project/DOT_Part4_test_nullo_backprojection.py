# =============================================================================
# TEST NULLO: la macchia "coerente" a lambda alto e' guidata dal segnale
# vero o e' un artefatto della forma dei primi modi di Tikhonov?
# =============================================================================
# A lambda grande, il filtro di Tikhonov f_i = s_i^2/(s_i^2+lambda^2) tiene
# vivi solo i primissimi modi (quelli meglio condizionati): la ricostruzione
# degenera in una backprojection A_hat(lambda->inf) ~ (1/lambda^2) W^T M, che
# non amplifica mai il rumore (non divide mai per s_i piccoli) ma restituisce
# comunque una macchia smussata e spazialmente coerente - PERCHE' i primi
# modi v_i hanno una loro forma spaziale intrinseca, fissata dalla sola
# geometria di W, indipendente da cosa contenga M.
#
# Il sospetto: quella macchia "al posto giusto" vista nella ricostruzione
# reale potrebbe non essere trainata dal segnale, ma essere semplicemente
# la forma che i primi modi impongono a QUALUNQUE vettore di misura
# (rumore incluso). Per verificarlo si ripete l'identica pipeline
# (stessa W, stesso rumore poissoniano sulla baseline) ma con un fantoccio
# SENZA alcuna inclusione (A_null = 0): se a parita' di lambda esce comunque
# una macchia simile (stessa forma/posizione), la macchia non e' evidenza
# di localizzazione - e' un artefatto strutturale del filtro.
# =============================================================================
...
# N_tot = 1e7 (10 milioni) per replicare esattamente il caso da cui e' nato
# il sospetto del test nullo (con 1e6 la macchia a lambda alto era gia'
# cosi' debole da essere meno impressionante da confrontare a occhio).
N_tot = 1e7
...
# --- CASO NULLO: stessa pipeline di rumore, ma A_null = 0 (nessuna
#     inclusione) -----------------------------------------------------------
# M_ideal_null e' identicamente zero (W @ 0 = 0): la "misura perturbata" ha
# quindi lo STESSO valore atteso della baseline. Le due acquisizioni
# poissoniane restano pero' indipendenti (stessa identica logica del caso
# reale), quindi M_noisy_null e' puro rumore di shot noise attorno a zero,
# senza alcuna struttura vera da recuperare.
A_null = np.zeros(N_vox)
M_ideal_null = np.zeros((N_pairs, N_gates))
Npert_ideal_null = N0_ideal * (1 + M_ideal_null)   # = N0_ideal

N0_noisy_null = np.random.poisson(N0_ideal)
Npert_noisy_null = np.random.poisson(np.clip(Npert_ideal_null, 0, None))

with np.errstate(divide='ignore', invalid='ignore'):
    M_noisy_null = np.where(N0_noisy_null > 0,
                             (Npert_noisy_null - N0_noisy_null) / N0_noisy_null,
                             np.nan)

print("Caso nullo (A=0): generato con la stessa pipeline di shot noise, "
      "due acquisizioni Poisson indipendenti (baseline e 'perturbata').")
...
M_noisy_null_flat = np.nan_to_num(M_noisy_null, nan=0.0).flatten()   # caso nullo (A=0)
...
# =============================================================================
# STEP 10 - TEST NULLO: stesso lambda_show, ma su M generato con A=0
# =============================================================================
# Stessa figura di prima, ma con due righe: in alto la ricostruzione dal
# dato REALE (inclusione + rumore), in basso la ricostruzione dallo STESSO
# tipo di rumore ma senza alcuna inclusione vera (A_null=0). Stessa scala
# colore per colonna (stesso lambda), cosi' l'ampiezza e' confrontabile a
# vista fra le due righe, non solo la forma.
n_cols = len(lambda_show) + 1
fig, axs = plt.subplots(2, n_cols, figsize=(4*n_cols, 8.0))

# --- riga 1: A vero e ricostruzioni dal dato reale ---
A_section_true = A_rep[:, :, iz_show2].T
axs[0, 0].imshow(A_section_true, origin='lower', cmap='viridis', vmin=0, vmax=dmu_a,
                  extent=(x_coords[0]-step/2, x_coords[-1]+step/2,
                          y_coords[0]-step/2, y_coords[-1]+step/2))
axs[0, 0].scatter(src_xy[:, 0], src_xy[:, 1], marker='*', c='red', s=90)
axs[0, 0].scatter(used_det_xy[:, 0], used_det_xy[:, 1], marker='o', c='cyan', s=25)
axs[0, 0].set_title('$A$ vero'); axs[0, 0].set_ylabel('y [mm]\n(dato REALE)')

# --- riga 2: A nullo (sempre zero) e ricostruzioni dal solo rumore ---
axs[1, 0].imshow(np.zeros_like(A_section_true), origin='lower', cmap='viridis', vmin=0, vmax=dmu_a,
                  extent=(x_coords[0]-step/2, x_coords[-1]+step/2,
                          y_coords[0]-step/2, y_coords[-1]+step/2))
axs[1, 0].scatter(src_xy[:, 0], src_xy[:, 1], marker='*', c='red', s=90)
axs[1, 0].scatter(used_det_xy[:, 0], used_det_xy[:, 1], marker='o', c='cyan', s=25)
axs[1, 0].set_title('$A$ nullo (nessuna inclusione)')
axs[1, 0].set_xlabel('x [mm]'); axs[1, 0].set_ylabel('y [mm]\n(TEST NULLO)')

for col, lam in enumerate(lambda_show, start=1):
    A_hat_real = tikhonov_solve(M_noisy_flat, lam)
    A_hat_null = tikhonov_solve(M_noisy_null_flat, lam)

    # stessa scala colore per le due righe della stessa colonna, cosi'
    # l'ampiezza (non solo la forma) e' confrontabile a vista
    vmax_col = max(np.abs(A_hat_real).max(), np.abs(A_hat_null).max())

    section_real = A_hat_real.reshape(x_coords.size, y_coords.size, z_coords.size)[:, :, iz_show2].T
    err_real = np.linalg.norm(A_hat_real - A) / np.linalg.norm(A)
    im = axs[0, col].imshow(section_real, origin='lower', cmap='RdBu_r', vmin=-vmax_col, vmax=vmax_col,
                    extent=(x_coords[0]-step/2, x_coords[-1]+step/2,
                            y_coords[0]-step/2, y_coords[-1]+step/2))
    axs[0, col].scatter(src_xy[:, 0], src_xy[:, 1], marker='*', c='k', s=90)
    axs[0, col].scatter(used_det_xy[:, 0], used_det_xy[:, 1], marker='o', c='k', s=18, alpha=0.5)
    axs[0, col].set_title(f'$\\hat A$ (reale), $\\lambda$={lam:.1e}\nerr={err_real:.2f}')
    fig.colorbar(im, ax=axs[0, col], shrink=0.8)

    section_null = A_hat_null.reshape(x_coords.size, y_coords.size, z_coords.size)[:, :, iz_show2].T
    im = axs[1, col].imshow(section_null, origin='lower', cmap='RdBu_r', vmin=-vmax_col, vmax=vmax_col,
                    extent=(x_coords[0]-step/2, x_coords[-1]+step/2,
                            y_coords[0]-step/2, y_coords[-1]+step/2))
    axs[1, col].scatter(src_xy[:, 0], src_xy[:, 1], marker='*', c='k', s=90)
    axs[1, col].scatter(used_det_xy[:, 0], used_det_xy[:, 1], marker='o', c='k', s=18, alpha=0.5)
    axs[1, col].set_title(f'$\\hat A$ (NULLO), $\\lambda$={lam:.1e}')
    axs[1, col].set_xlabel('x [mm]')
    fig.colorbar(im, ax=axs[1, col], shrink=0.8)

fig.suptitle(f"Test nullo: ricostruzione da dato reale (sopra) vs da solo rumore, A=0 (sotto)\n"
             f"stessa scala colore per colonna - $z\\approx${z_coords[iz_show2]:.0f} mm, "
             f"vero: $x_p$={xp:.0f}, $y_p$={yp:.0f}, $z_p$={zp:.0f} mm  "
             f"(N_tot={N_tot:.0e}, {N_src} sorgenti, {N_pairs} coppie)")
fig.tight_layout()
plt.show()

print("="*55)
print("CONFRONTO LOCALIZZAZIONE: dato REALE vs TEST NULLO (stesso lambda)")
print("="*55)
print(f"Vero voxel perturbato:  ({xp:.1f}, {yp:.1f}, {zp:.1f}) mm")
dist_true = np.linalg.norm(r_V - np.array([xp, yp, zp]), axis=1)
for lam in lambda_show:
    A_hat_real = tikhonov_solve(M_noisy_flat, lam)
    A_hat_null = tikhonov_solve(M_noisy_null_flat, lam)

    peak_real = r_V[np.argmax(A_hat_real)]
    peak_null = r_V[np.argmax(A_hat_null)]
    amp_real = A_hat_real.max()
    amp_null = A_hat_null.max()
    dist_null_from_true = dist_true[np.argmax(A_hat_null)]

    print(f"lambda={lam:.2e}:")
    print(f"  REALE:  picco = ({peak_real[0]:5.2f}, {peak_real[1]:5.2f}, {peak_real[2]:5.2f}) mm   "
          f"ampiezza picco = {amp_real:.3e}")
    print(f"  NULLO:  picco = ({peak_null[0]:5.2f}, {peak_null[1]:5.2f}, {peak_null[2]:5.2f}) mm   "
          f"ampiezza picco = {amp_null:.3e}   "
          f"(distanza dal vero: {dist_null_from_true:.1f} mm)   "
          f"| ampiezza nullo/reale = {amp_null/amp_real:.2f}")
print("="*55)
print("Se 'ampiezza nullo/reale' e' vicino a 1 e il picco del test nullo cade")
print("comunque in una macchia unica e compatta (non rumore incoerente diffuso),")
print("la macchia 'coerente' vista nel caso reale NON e' evidenza affidabile di")
print("localizzazione: e' la forma che i primi modi di Tikhonov impongono a")
print("QUALUNQUE dato, rumore puro incluso.")
print("="*55)
