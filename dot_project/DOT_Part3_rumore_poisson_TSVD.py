# -*- coding: utf-8 -*-
"""
DOT Project - Parte 3: rumore poissoniano (shot noise) sul vettore delle
misure M e ricostruzione TSVD. Versione originale, analizzata in
DOT_Part3_rumore_poissoniano.ipynb.
"""
import numpy as np
...
A_rep = mask * dmu_a           # maschera booleana, valido finxhè l'inclusione è cubica
...
# M = dPhi/Phi0 (Step 3) e' un rapporto normalizzato fra due misure di
# fluenza integrata nei gate, NON un segnale "grezzo". La grandezza
# fisicamente rumorosa in un sistema TCSPC e' il numero di fotoni
# rivelati per gate: processo di conteggio discreto, la cui statistica e'
# Poissoniana (P(k) = lambda^k * exp(-lambda) / k!, media = varianza =
# lambda). Lo shot noise è ma la conseguenza intrinseca della natura discreta
# della rivelazione di
# fotoni, quindi irriducibile. Il SNR di una misura poissoniana scala
# come sqrt(lambda): per questo i gate con pochi conteggi (code precoci e
# tardive della TPSF, spesso i piu' sensibili a inclusioni profonde) sono
# quelli piu' rumorosi e piu' lontani dal regime gaussiano (CLT, valido
# solo per lambda grande).
#
# Per rumorare M in modo corretto bisogna quindi tornare a livello di
# conteggi, applicare li' il rumore, e ricalcolare il rapporto - non
# aggiungere rumore direttamente a M.

np.random.seed(42)  # riproducibilita'

# --- 1) Conteggi ideali di riferimento (mezzo omogeneo, senza inclusione) ---
# Ricalcolo dell'integrale di phi0 per gate e rivelatore (nello Step 3
# viene usato per normalizzare le righe di W ma non viene salvato)
...
# --- 2) Budget fotonico totale per rivelatore (tipico sistema TCSPC) ---
# Ordine di grandezza realistico: 1e4-1e6 conteggi totali per curva TPSF.
# I fotoni vengono distribuiti fra i gate in proporzione alla forma di
# phi0(t): piu' conteggi dove la TPSF e' alta (vicino al picco), pochi
# nelle code, esattamente come avviene in un'acquisizione reale.
N_tot = 1e6
N0_ideal = N_tot * Phi0_int / Phi0_int.sum(axis=1, keepdims=True)

# --- 3) Conteggi ideali della misura perturbata (con inclusione) ---
# M_ideal = dPhi/Phi0  =>  Phi_pert = Phi0 * (1 + M_ideal)
Npert_ideal = N0_ideal * (1 + M_ideal)

# --- 4) Realizzazioni poissoniane indipendenti delle due acquisizioni ---
# Baseline e misura perturbata sono due acquisizioni distinte: ciascuna
# ha la propria statistica di shot noise, indipendente dall'altra.
N0_noisy = np.random.poisson(N0_ideal)
Npert_noisy = np.random.poisson(np.clip(Npert_ideal, 0, None))

# --- 5) Dato normalizzato rumoroso, come si calcolerebbe da dati reali ---
with np.errstate(divide='ignore', invalid='ignore'):
    M_noisy = np.where(N0_noisy > 0,
                        (Npert_noisy - N0_noisy) / N0_noisy,
                        np.nan)

print(f"Budget fotonico: N_tot = {N_tot:.0e} conteggi/rivelatore")
...
fig.suptitle(f'Confronto M ideale vs M con shot noise '
             f'(N_tot = {N_tot:.0e} conteggi/rivelatore)')
plt.show()

# --- Plot 2: profilo di un singolo rivelatore, gate per gate ---
d_show = 0
plt.figure(figsize=(7, 4))
plt.plot(range(N_gates), M_ideal[d_show], 'o-', label='M ideale')
plt.plot(range(N_gates), M_noisy[d_show], 's--', label='M con rumore')
plt.xlabel('indice gate temporale')
plt.ylabel('C (-)')
plt.title(f'Rivelatore D{d_show + 1}: confronto ideale vs rumoroso')
plt.legend()
plt.grid()
plt.show()
...
# NOTA: a differenza della versione "senza rumore", qui il problema inverso
# viene risolto anche a partire da M_noisy (Step 5): con dati rumorosi,
# dividere per valori singolari s_i piccolissimi amplifica il rumore
# invece di recuperare segnale, quindi ci si aspetta un comportamento
# qualitativamente diverso dal caso ideale - non piu' "piu' k e' sempre
# meglio", ma un compromesso bias/varianza con un minimo a k intermedio
# (il principio della regolarizzazione per troncamento).
M_ideal_flat = M_ideal.flatten()          # stesso ordine (d*N_gates+g) delle righe di W
M_noisy_flat = np.nan_to_num(M_noisy, nan=0.0).flatten()   # eventuali NaN (baseline
                                                            # a conteggio zero) -> 0
...
rel_res_ideal = np.zeros_like(k_values, dtype=float)
rel_res_noisy = np.zeros_like(k_values, dtype=float)
...
for j, k in enumerate(k_values):
    ...
    rel_res_ideal[j] = np.linalg.norm(W @ A_hat_i - M_ideal_flat) / np.linalg.norm(M_ideal_flat)
    rel_res_noisy[j] = np.linalg.norm(W @ A_hat_n - M_noisy_flat) / np.linalg.norm(M_noisy_flat)

k_best = k_values[np.argmin(rel_err_noisy)]   # k "oracolo": nella pratica non si
...
plt.title('Troncamento SVD: ideale (monotono) vs con rumore (minimo a k finito)')
